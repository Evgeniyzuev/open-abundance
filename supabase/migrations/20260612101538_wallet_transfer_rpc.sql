...
begin
  ...
  transfer_source_id := coalesce(p_source_id, gen_random_uuid());
  sender_key := case when p_idempotency_key is null or p_idempotency_key = '' then null else p_idempotency_key || ':debit' end;
  recipient_key := case when p_idempotency_key is null or p_idempotency_key = '' then null else p_idempotency_key || ':credit' end;

  if sender_key is not null then
    select id
    into existing_sender_ledger_id
    from public.wallet_ledger
    where idempotency_key = sender_key
    limit 1;

    select id
    into existing_recipient_ledger_id
    from public.wallet_ledger
    where idempotency_key = recipient_key
    limit 1;
  ...
  if not found then
    raise exception 'Sender Wallet is not created yet.';
  end if;

  select *
  into recipient_wallet
  from public.wallet_accounts
  where user_id = p_recipient_user_id;

  if not found then
    raise exception 'Recipient Wallet is not created yet.';
  end if;
  ...
  set balance = next_sender_balance,
      updated_at = now()
  where user_id = p_sender_user_id;

  update public.wallet_accounts
  set balance = next_recipient_balance,
      updated_at = now()
  where user_id = p_recipient_user_id;
  ...
  values (
    ...
    'credit',
    p_amount,
    recipient_wallet.currency_code,
    'wallet_transfer',
    transfer_source_type,
    transfer_source_id,
    p_sender_user_id,
    next_recipient_balance,
    recipient_key,
    jsonb_build_object(
      'transfer_role',
      'recipient'
...
revoke all on function public.wallet_transfer(uuid, uuid, numeric, text, uuid, text) from public;
revoke all on function public.wallet_transfer(uuid, uuid, numeric, text, uuid, text) from anon;
revoke all on function public.wallet_transfer(uuid, uuid, numeric, text, uuid, text) from authenticated;
