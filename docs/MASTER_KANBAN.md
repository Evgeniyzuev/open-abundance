...
**DEV-WISH-001 — пройти первый маршрут от демо-истории до личного пути без объяснений.** Ответственный за проверку: Основатель. Статус: На проверке. Technical QA завершён; после успешного User QA пакет набора становится главным действием.

Маршрут использует существующие ленту, рекомендации, желания и Home. Подробные этапы и ограничения находятся в [плане ключевого цикла](WISH_JOURNEY_CORE_LOOP_PLAN.md).
...
| DEV-WISH-001 / разработка | Демо-история → личное желание → ближайшее желание → путь на Home | Основатель | На проверке | Исправление первого User QA подготовлено; нужен повторный мобильный проход | Повторить «Хочу так же»; при успехе выбрать «Сделать ближайшим», на Home записать проверяемый результат и нажать «Сохранить ориентир» | Желание создаётся либо рядом с действием видна ошибка; путь открывается после явного выбора; ориентир сохраняется после перезагрузки | [План ключевого цикла](WISH_JOURNEY_CORE_LOOP_PLAN.md); 2026-09-09: первый User QA выявил пропавший результат и неясные шаги; remote-каталог и откатываемая вставка проверены, исправлены refresh сессии, видимость ошибки и подсказки; повторный User QA не пройден |
| PILOT-001 / подготовка | Приглашение, сценарий разговора и десять карточек заданий | Codex | Ожидает | DEV-WISH-001 прошёл User QA; согласованный объём пилота | После проверки маршрута подготовить пакет для основателя | В каждой карточке есть результат, proof, награда и ответственный за приёмку | Пока отсутствует; требования в [плане пилота §2–3](PILOT_30_DAY_PLAN.md#2-порядок-подготовки-и-набора) |
| OPS-001 / привлечение | Десять разговоров и пять согласившихся участников | Основатель | Ожидает | PILOT-001 | Выбрать адресатов после готовности приглашения | Отдельно подтверждены отправки, разговоры и согласие пяти людей | Пока отсутствует; только агрегаты в этом трекере |
...
| OPS-002 / сопровождение | Запущена первая группа и зафиксирована дата начала 30 дней | Основатель | Ожидает | OPS-001, DEV-001, FIN-002; допуск к наградам | После готовности провести первый сопровождаемый сеанс | Все пять прошли старт; критерии допуска выполнены, дата записана | Пока отсутствует |
...
|---|---|---|---|
| OPS-001 | Пакет набора ещё не подготовлен | Codex | PILOT-001 готов к использованию; отправка остаётся действием основателя |
...
Ближайшая последовательность: DEV-WISH-001 → PILOT-001 → OPS-001 и DEV-001; FIN-001 → FIN-002; после их завершения OPS-002 → OPS-003. Работы могут идти параллельно только в пределах WIP-ограничений ниже.
...
| TEAM-001 / проверки | Подтвердить Team Help Loop | Основатель | На проверке | Ручной сценарий двух участников | Пройти помощь и командное взаимодействие | Результат ручного QA записан | [План команд](REFERRALS_TEAMS_PLAN.md); remote apply записан ранее |
