...
2026-09-12: локальный интерфейс Челленджей переведён на послойный обзор. Today стал первым компактным элементом «Доступных» и раскрывает существующие прогресс, серии, чек-лист и проверку внутри списка. Остальные строки показывают название и награду; детали сохранены в модальном экране. Визуальные и общие решения описаны в [ENGAGING_INTERFACE_PLAN.md](ENGAGING_INTERFACE_PLAN.md). Требуется User QA.
...
- Challenges screen показывает Today первым компактным элементом «Доступных»; прогресс `$current / $target`, streak и чек-лист раскрываются по нажатию.
