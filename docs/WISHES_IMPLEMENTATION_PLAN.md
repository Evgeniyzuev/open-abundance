...
## Пирамида глубины желаний

Смысловая модель из пяти слоев `Безопасность -> Свобода -> Реализация -> Признание -> Единство` зафиксирована в [`WISH_PROGRESS_PYRAMID.md`](./WISH_PROGRESS_PYRAMID.md).

Рекомендуемое будущее место — необязательный блок `Goals -> Wishes -> Моя пирамида желаний` и опциональная классификация при создании желания. Модель не смешивается с Core-уровнями, difficulty, reward или публичным рейтингом. До отдельного UI-прототипа и User QA новые поля в `wishes` не добавляются.
...
Статус 2026-07-24: реализована первая часть completion flow. Экран желаний разделён на полноширинные вкладки `Рекоменд. / Мои / Исполнено`; после отметки желание сразу переходит в `Исполнено`. Действие переименовано в «Исполнено» и открывает явный выбор между завершением без публикации и публичным постом о результате. Completion-post идемпотентен по `source_key`, публикует только имя автора из профиля и название желания, не раскрывая остальные поля private wish. Исправлена UUID-валидация `PATCH /api/wishes/[wishId]`, из-за которой корректный id отклонялся как `Invalid wish id`.
...
Экран личных желаний переведён на плотную трёхколоночную сетку `4:5`: минимальные зазоры, заголовок поверх изображения и единый вид для рекомендаций, личных и выполненных желаний. CRUD, приватность и completion flow не менялись.

Статус 2026-09-01: каталог `recommended_wishes` расширен восемью двуязычными шаблонами (активы $1 000 000, переезд, финансовая подушка, работа по душе, своё дело, семья, кругосветное путешествие и финансовая поддержка родителей). В `WishesApp` исправлен разбор группированных и десятичных `estimated_cost`, чтобы добавление рекомендованной цели сохраняло корректную сумму.
