...
**DEV-UX-001 — упростить ключевые экраны перед набором в пилот.** Ответственный: Codex; проверяющий: Основатель. Статус: На проверке. Версия 2 реализована локально для Челленджей, Home и Новы; технические проверки прошли. Нужен мобильный User QA основных сценариев. Авторские изображения, 3D и анимации не блокируют пилот.

Общий принцип, последовательность и критерии находятся в [плане увлекательного интерфейса](ENGAGING_INTERFACE_PLAN.md). Подтверждённый маршрут желаний сохраняется как регрессионный путь.
...
| DEV-UX-001 / разработка | Понятный выбор челленджа, один следующий шаг на Home и удобная переписка с Новой | Codex / Основатель | На проверке | DEV-WISH-001 подтверждён; решение основателя упростить UI до набора | Пройти мобильный User QA Челленджей, Home и Новы, включая регрессию маршрута желания | Участник без подсказок выбирает доступное задание, продолжает желание, отличает проверку от начисления и возвращается без потери текста/места; мобильный и доступный User QA, регрессия DEV-WISH-001 | [План интерфейса, версия 2](ENGAGING_INTERFACE_PLAN.md); 2026-09-12: версия 2 реализована локально — честные награды и видимые статусы, fallback при сбое изображения, один CTA Home, свёрнутые вторичные блоки, редактируемые подсказки и компактный диалог Новы. TypeScript, lint, production build, целевая детерминированная проверка, UTF-8, ссылки и `git diff --check` прошли. Browser на 390 × 844 подтвердил только оболочку и навигацию; данные Челленджей не загрузились из-за локальных API-ошибок, поэтому визуальный и User QA не подтверждены. 2026-09-13: обычные челленджи раскрываются внутри списков, форма сохраняется при сворачивании; TypeScript, lint и build прошли; браузерная попытка увидела прежний сервер на 3100, новую разметку визуально не подтвердила |
| PILOT-001 / подготовка | Приглашение, сценарий разговора и десять карточек заданий | Codex | Ожидает | DEV-UX-001 прошёл User QA; согласованный объём пилота | После упрощения ключевых экранов подготовить пакет для основателя | В каждой карточке есть результат, proof, награда и ответственный за приёмку | Пока отсутствует; требования в [плане пилота §2–3](PILOT_30_DAY_PLAN.md#2-порядок-подготовки-и-набора) |
...
Ближайшая последовательность: DEV-WISH-001 подтверждён → DEV-UX-001 → PILOT-001 → OPS-001 и DEV-001; FIN-001 → FIN-002; после их завершения OPS-002 → OPS-003. Работы могут идти параллельно только в пределах WIP-ограничений ниже.
