...
type CryptoDepositLedgerRow = {
  ...
  operation_type: "crypto_deposit" | "crypto_withdrawal" | "challenge_reward" | "wallet_core_topup" | "wallet_transfer" | "marketplace_escrow_hold" | "marketplace_payment" | "marketplace_refund";
  direction: "credit" | "debit";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: cryptoLedger, error: cryptoError } = await supabase
      ...
      .eq("user_id", user.id)
      .in("operation_type", ["crypto_deposit", "crypto_withdrawal", "challenge_reward", "wallet_core_topup", "wallet_transfer", "marketplace_escrow_hold", "marketplace_payment", "marketplace_refund"])
