...
export function PublicBlog({ authorId, locale, active = true, revision = "", t, onOpenPost }: {
  ...
  return <section className="blog-public" aria-label={t("social.blog.authorPosts")}>
    ...
    {payload?.posts.length ? <FeedPostGallery fallbackTitle={t("social.post.detail")} posts={payload.posts} onOpen={onOpenPost} /> : !loading && payload ? <p>{t("social.blog.empty")}</p> : null}
