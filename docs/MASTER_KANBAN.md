...
- **Статус:** Сейчас — 23 server-backed demo posts реализованы, 10 согласованных demo-текстов обновлены, 12 system stories опубликованы; профиль `Abundance System` и real verified `Challenge Done` остаются следующими срезами.
...
- **Системный аккаунт и порядок:** в БД создан `Abundance System`, главы имеют server-backed порядок `1–12`, тип `system_story`, отдельный бейдж и фото 4:5; переход из карточки в отдельный профиль аккаунта пока остается следующим UI-срезом. Подробности — `docs/REALITY_FEED_SYSTEM_STORIES_PLAN.md`.
- **Граница текущего шага:** системные истории объясняют позицию проекта, но не получают verified badge; отдельный source pack, профиль системного аккаунта и real `Challenge Done` выполняются следующими независимыми срезами.
- **Изменения:**
