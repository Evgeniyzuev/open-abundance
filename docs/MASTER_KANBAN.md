...
- [x] **Единый onboarding:** 2026-07-20 первое знакомство упрощено до трех экранов `миссия -> истории -> программа 20 уровней` без форм и оговорок; добавлены отдельные яркие иллюстрации, CTA расчета ведет в доступный гостю Core-калькулятор, альтернативный CTA — к первому заданию. Повторный вход без onboarding сохраняется; полноценное создание server-side Wish/GrowthPlan остается отдельной задачей.
