...
**DEV-UX-001 — упростить ключевые экраны перед набором в пилот.** Ответственный: Codex; проверяющий: Основатель. Статус: На проверке. Первый срез Челленджей реализован локально и ждёт мобильного User QA; следующие срезы — Home и Нова.
...
| DEV-UX-001 / разработка | Послойные и визуально привлекательные Челленджи, Home и Нова | Codex / Основатель | На проверке | DEV-WISH-001 подтверждён; решение основателя упростить UI до набора | Пройти мобильный User QA первого среза Челленджей, затем реализовать компактный маршрут Home | Обзор понятен за пять секунд; детали открываются по выбору; одно главное действие; мобильный и доступный User QA | [План интерфейса](ENGAGING_INTERFACE_PLAN.md); 2026-09-12: локально реализованы новое название, Today в «Доступных», компактные строки и игровые fallback-визуалы; TypeScript, lint, build, UTF-8, ссылки и `git diff --check` прошли; единственная browser-попытка остановлена после ответа `No browser is available`, User QA не пройден |
