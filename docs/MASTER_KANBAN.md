...
| DEV-WISH-001 / разработка | Демо-история → личное желание → ближайшее желание → путь на Home | Основатель | На проверке | Technical QA завершён; нужен ручной мобильный User QA | Без объяснений пройти историю → «Хочу так же» → «Сделать ближайшим» → Home; повторить добавление и перезагрузить страницу | Шесть связей, защита от дубля, явный выбор, честный расчёт и минутный шаг работают; участник различает демонстрацию, Wallet, Core и реальный результат | [План ключевого цикла](WISH_JOURNEY_CORE_LOOP_PLAN.md); 2026-09-09: typecheck, lint, production build и `git diff --check` пройдены; браузерная среда недоступна, User QA не приписан |
