...
| TEAM-002 / разработка | Teams как круг роста: договорённости, ежедневники, комната и обмен приёмами | Codex / Основатель | На проверке | TEAM-001 сохраняет отдельный User QA; authenticated профиль для проверки | Пройти User QA UI-среза и затем начать этап 2: договорённость и запрос помощи | UI-срез этапа 1, технические проверки и User QA обеих ролей; этапы 2–5 выпускаются отдельно, 6–8 — последующие | [Концепция и этапы кода](TEAMS_GROWTH_CIRCLE_PLAN.md); 2026-09-14: добавлен локальный TeamsGrowthHero с фокусом ролей, комнатой и сводкой действия; миграции не менялись, remote не проверялся |
