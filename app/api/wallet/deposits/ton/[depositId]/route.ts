...
export async function GET(request: NextRequest, { params }: { params: { depositId: string } }) {
  try {
    ...
    const { data: events, error: eventsError } = await supabase
      ...
      .select("id,network,asset_code,transaction_hash,logical_time,message_index,sender_address,receiver_address,amount_nano,comment,invoice_code,status,finalized_at,ton_usd_rate,rate_provider,rate_source_timestamp,settled_usd_amount,settled_at,created_at,updated_at")
