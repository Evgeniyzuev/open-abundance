...
6. Загруженные картинки Tasks MVP пока храним локально, синхронизацию добавляем позже. Уточнение 2026-08-01: это решение относится к task attachments и не является глобальным запретом Storage; Feed использует существующий private Supabase `feed-media` как ограниченный MVP adapter, затем может перейти на S3-compatible storage по `FEED_POSTING_RECOMMENDATIONS_PLAN.md`.
