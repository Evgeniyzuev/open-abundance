...
- **Изменения:** `first_core_path`, шаги/prerequisites/action views, только Core-награды, серверная защита порядка, отдельный UI-блок прогресса и first-party funnel events.
...
- **Критерии приемки:** пять шагов отображаются по порядку; следующий шаг недоступен до предыдущего; bypass через API возвращает 409; завершение создает Core-награду один раз; события воронки записываются без блокировки пользовательского действия.
